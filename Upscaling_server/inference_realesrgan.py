...
def main():
    ...
    print(f"Inference started at: {start_time}")

    for idx, path in enumerate(paths):
        imgname, extension = os.path.splitext(os.path.basename(path))
        
        # Extract numeric part and check if it satisfies the condition
        try:
            num_part = int(''.join(filter(str.isdigit, imgname)))  # Extract numeric part
            if num_part % 3 != 0:  # Skip if remainder is not 1 when divided by 3
                continue
        except ValueError:
            print(f"Skipping file without numeric part in filename: {path}")
            continue
        ...
        if img_mode == 'RGBA':  # RGBA images should be saved in png format
            extension = 'png'
        ### 저장 디렉토리 ###
        save_path = f'/outputvideo/hls1_out/{imgname}_{args.suffix}.{extension}'
    ...
    print(f"Inference ended at: {end_time}")
